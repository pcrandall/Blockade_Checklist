window.addEventListener('load', function () {
 ...
 _failed = (e) => {
  ...
  _target.addEventListener('drop', function (event) {
    file = event.dataTransfer.files[0].name
    n = file.replace(/\.[^/.]+$/, "")
    filename = n + "_complete"
    console.log({"name": n})
  });
 ...
 _onsheet = (json) => {
    ...
    if (_file.value !== "") {
      filename = _file.value.slice(12, _file.value.length - 5) + "_complete";
    } else if (filename === null){
      filename =  'Blockade_Checklist_complete'
    }

    wbName.value = filename;
    ...
    let tmp = Object.values(sortedCols);
    let found = [];
    tmp.forEach(  (col) => {
      var key = col.index;
